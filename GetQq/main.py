...
"""
此脚本是为了满足我不想在手机上下载qq所写
一直以来我都觉得QQ的界面越来越臃肿, 且随着QQ使用时间的增加各种群各种联系人越来越多, 
...
def send_qq(name):
    ...
    with open('new_pic.png', 'rb') as f:
        # 设置附件的MIME和文件名，这里是png类型:
        mime = MIMEBase('image', 'png', filename='new_pic.png')
        # 加上必要的头信息:
        mime.add_header('Content-Disposition', 'attachment', filename='new_pic.png')
...
def main(name):
    ...
    match_result = compare_images("old_pic.png", "new_pic.png")
    if not match_result:
        print("发现新消息：发送邮件")
        ret = send_qq(name)
        if ret:
